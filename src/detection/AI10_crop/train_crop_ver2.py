...
intermediate_layer_model = Model(inputs=complessed_mobilenet.input,
                                 outputs=complessed_mobilenet.get_layer(layer_name).output)

# 中間層を追加
x = GlobalAveragePooling2D()(intermediate_layer_model.output)
x = BatchNormalization()(x)  # バッチ正規化を追加
x = Dropout(0.5)(x)  # ドロップアウトを追加して過学習を防ぐ

# 全結合層を追加して (x, y) 座標を出力
output = Dense(2, activation='sigmoid')(x)  # 2つのニューロンで (x, y) 座標を出力

# 新しいモデルを構築
model_with_coordinates = Model(inputs=intermediate_layer_model.input, outputs=output)
