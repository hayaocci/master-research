...
def render_single_img(save_dir_path, img_name):
    bpy.context.scene.render.filepath = save_dir_path + "/" + img_name + ".png"
    bpy.context.scene.render.image_settings.file_format = 'PNG' 
    bpy.ops.render.render(write_still=True)

def render_img(input_dir, output_dir, num_pic):
    make_dir()

    # 正解ラベル画像の保存先ディレクトリに存在する画像の数を取得
    file_count = get_file_num(output_dir)

    if file_count < num_pic:
...
def render_main2():
    ...
    render_img(TRAIN_INPUT_DIR, TRAIN_OUTPUT_DIR, TRAIN_PIC)

    render_img(VALID_INPUT_DIR, VALID_OUTPUT_DIR, VALID_PIC)
